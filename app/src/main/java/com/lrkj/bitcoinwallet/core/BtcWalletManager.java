...
public class BtcWalletManager {
    ...
    private BtcWalletManager(@NonNull NetworkParameters networkParameters, @NonNull AssetManager assetManager, @NonNull File directory) {
        Log.d(TAG, "BtcWalletManager: 创建对象");
    ...
    public static String generateMnemonic() {
        ...
        final DeterministicSeed seed = new KeyChainGroup(Constants.NETWORK_PARAMETERS).getActiveKeyChain().getSeed();
    ...
    private void create(@NonNull DeterministicSeed seed, @NonNull CompletableEmitter emitter) {
        ...
        try {
            walletAppKit.setCheckpoints(assetManager.open(Constants.NETWORK_PARAMETERS.equals(TestNet3Params.get()) ? "checkpoints-testnet.txt" : "checkpoints.txt"));
        } catch (IOException e) {
            e.printStackTrace();
        }
